    ...
    const CustomOrder = (Order) =>
        class extends Order {
            get_total_without_discount_lines() {
                const normalOrderLines = this.orderlines.filter(
                    (orderline) => orderline.product.default_code !== "DISC"
                );
                return round_pr(
                    normalOrderLines.reduce(function (sum, orderLine) {
            ...
            get_total_of_discount_lines() {
                const discountOrderLines = this.orderlines.filter(
                    (orderline) => orderline.product.default_code === "DISC"
                );
                return round_pr(
                    discountOrderLines.reduce(function (sum, orderLine) {
                        return sum + orderLine.get_price_with_tax();
                    }, 0),
                    this.pos.currency.rounding
                );
            ...
            export_for_printing() {
                ...
                receipt.total_without_discount_lines =
                    this.get_total_without_discount_lines();
